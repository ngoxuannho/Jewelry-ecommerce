                            ...
                            <tbody>
                                ...
                                {dataCart && dataCart.length > 0 &&
                                    dataCart.map((item, index) => {
                                        ...
                                        let name = "";
if (item.productData && item.productDetail && item.productdetailsizeData && item.productdetailsizeData.sizeData) {
    name = `${item.productData.name} - ${item.productDetail.nameDetail} - ${item.productdetailsizeData.sizeData.value}`;
} else {
    // Xử lý khi một hoặc nhiều thuộc tính không tồn tại
