            ...
            <div className="product_top_bar">
                <div className="left_dorp">
                    ...
                    <select style={{ outline: 'none' }} onChange={(event) => handleSelectLimitPage(event)} className="show">
                        <option value={6}>Hiển thị 5</option>
                        <option value={12}>Hiển thị 10</option>
                        <option value={18}>Hiển thị 100</option>
                    </select>
                    ...
                    {dataProduct && dataProduct.length > 0 &&
                        dataProduct.map((item, index) => {
                            return (
                                ...
                                name={item.name}
                                img={item.productDetail[0]?.productImage[0]?.image}  // Thêm dấu "?" để tránh lỗi khi truy cập
                                discountPrice={item.productDetail[0]?.discountPrice} // Thêm dấu "?" để tránh lỗi khi truy cập
                                price={item.productDetail[0]?.originalPrice} // Thêm dấu "?" để tránh lỗi khi truy cập
                            ></ItemProduct>
                            )
                        })
                    }
